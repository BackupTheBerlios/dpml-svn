...
public class BlockTask extends ProjectTask
{
    ...
    */
    public void execute()
    {
        ...
        if( m_standalone )
        {
            File file = getStandaloneFile();
            execute( file, true ); // generate the standalone block
        }
    }

    public void execute( File file, boolean standalone ) throws BuildException
    {
        ...
        try
        {
            ...
            try
            {
                writePluginDef( output, def, standalone );
            }
            finally
            {
                closeStream( output );
            ...
            String gpg = getIndex().getProperty( net.dpml.magic.project.Context.GPG_EXE_KEY );
    ...
    protected File getStandaloneFile()
    {
        final File dir = getContext().getDeliverablesDirectory();
        final File types = new File( dir, "blocks" );
        final Definition def = getDefinition();
        final Info info = def.getInfo();
        final String filename = info.getShortFilename() + ".block";
        return new File( types, filename );
    }

    protected File getEmbeddedFile()
    {
    ...
    private void writePluginDef( final OutputStream output, final Definition def, boolean standalone )
        throws IOException
    {
        final Writer writer = new OutputStreamWriter( output );
        writeHeader( writer );
        writePlugin( writer, def, standalone );
        writer.flush();
    }
    ...
    */
    ...
    protected void writePlugin( final Writer writer, final Definition def, boolean standalone )
        throws IOException
    {
        writeContainer( "", writer, def, standalone );
    }

    protected void writeContainer(
      final String pad, final Writer writer, final Definition def, boolean standalone )
        throws IOException
    {
        ...
        throws IOException
    {
        writer.write(
          "\n\n" + pad + "<include name=\""
          + include.getName() + "\" uri=\"" + include.getURI() + "\"" );
    ...
    */
    public static class Include extends Identifiable
    {
        ...
        */
        public void setUri( final String spec )
        {
            m_artifact = spec;
        }
